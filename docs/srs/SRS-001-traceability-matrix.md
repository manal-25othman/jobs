...
**الإصدار:** 1.0 · **مرتبط بـ:** `SRS-001-career-os-functional.md` v1.1 · **القرار:** D-039
...
| FR-A-012 | INV-3 | CAP-37 | *(مصدر: D-035 / عقود الوكلاء)* | A01–A19 | P1 | MUST | دمج تنفيذ مسموح · دمج عقود ممنوع | TC-TBD-082 | **GAP-05** |
...
| FR-G-018 | P-TRUST | CAP-42 | *(مصدر: D-036)* | P1 | MUST | Admin | لا تأثير تقييمي لـAdmin | TC-TBD-100 | **GAP-05** |
| FR-G-019 | P-TRUST | CAP-42 | *(مصدر: D-036)* | P1 | MUST | Admin | قراءة بسبب موثّق + حدث تدقيق | TC-TBD-101 | **GAP-05** |
| FR-G-020 | P-TRUST | CAP-11 | *(مصدر: D-030)* | P0/P1 | MUST | SME | لا نشر بلا اعتماد SME | TC-TBD-102 | **GAP-06** |

---

# 5. سجل الفجوات (Gap Register)

> **مبدأ D-039:** الفجوة تُعرض ولا تُخفى. كل فجوة لها اقتراح حل، ولا تُغلق إلا بقرار تغيير (`CHG-xxx`).

| Gap ID | النوع | الوصف | الأثر | الحل المقترح | الحالة |
|---|---|---|---|---|---|
| **GAP-00** | تناقض عددي | §19.2 في الـSRS ذكر `FR-U (41) · FR-P (24) · المجموع 97`. العدّ الفعلي: **FR-U 47 · FR-P 23 · FR-A 12 · FR-G 20 = 102** | إحصاء خاطئ في التتبّع | **تصحيح §19.2 إلى 102** — نُفِّذ في v1.1 | ✅ **مُصحَّح** |
| **GAP-01** | ميزة بلا متطلب | `F-CON-07` (3 أنشطة معتمدة لمسار واحد) لا يقابلها متطلب وظيفي — هي **التزام تسليم محتوى** لا سلوك نظام | يظهر النطاق أصغر مما هو | يُعامل كـ**التزام محتوى في Phase 0/P1** ويُتتبَّع في `27-team-resource-model` بدل FR | 🟡 **مقبول بتفسير — يُوثَّق** |
| **GAP-02** | ميزة بلا متطلب | **إنشاء `Achievement`** (`F-WRK-08` · `F-EVD-07` · CAP-29) بلا متطلب وظيفي صريح؛ مغطّى فقط بقاعدة عمل BR-007 وحالات SR-007 | قدرة حقيقية بلا متطلب قابل للاختبار | **إضافة `FR-P-036`**: يجب أن يشتق النظام `Achievement` حتميًا من `evidence.created` + `activity.completed`، وأن يمنع إنشاءه بلا دليل | 🔴 **يتطلب `CHG-001`** |
| **GAP-03** | ميزة بلا متطلب | **واجهة المراجع وسجل قرار المراجعة** (`F-EVL-13`) مغطّاة بسياسات HR-001..006 لا بمتطلب قدرة | لا متطلب قابل للاختبار لأداة يعتمد عليها ضمان الجودة | **إضافة `FR-P-037`**: يجب أن يوفّر النظام واجهة مراجعة عمياء تسجّل قرار المراجع بحقوله الإلزامية بما فيها `role_performed` (D-038) | 🔴 **يتطلب `CHG-002`** |
| **GAP-04** | تغطية جزئية | `F-EVD-02` (سلّم المستويات) و`F-EVD-05` (حالة الحداثة) مغطّيان عبر §8 و`SR-006` و`DR-012` لا عبر FR مستقل | مقبول — المستويات **قاعدة لا قدرة** | إبقاء التغطية عبر `SR-006` + `DR-012` + §8، مع الإشارة صراحة في المصفوفة | 🟡 **مقبول** |
| **GAP-05** | متطلب بلا ميزة | `FR-A-012` · `FR-G-018` · `FR-G-019` مصدرها **قرارات** (D-035 · D-036) لا مزايا في جرد Phase 1 | الجرد المجمّد لا يعكس ثلاثة متطلبات معتمدة | **إضافة ثلاث مزايا للجرد**: `F-GOV-12` تقييد Admin · `F-GOV-13` تسجيل قراءة Admin · `F-AI-11` استقلال عقود الوكلاء | 🔴 **يتطلب `CHG-003`** |
| **GAP-06** | متطلب بلا ميزة | `FR-G-020` (اعتماد SME) مصدره D-030 لا ميزة في الجرد | ضابط حاجب غير ممثَّل في الجرد | **إضافة `F-CON-12`**: بوابة اعتماد SME قبل نشر نشاط يمنح `Verified` | 🔴 **يتطلب `CHG-004`** |

**الخلاصة:** **4 فجوات تتطلب قرارات تغيير** (`CHG-001..004`) · فجوتان مقبولتان بتفسير موثّق · فجوة واحدة صُحِّحت في v1.1.

> **ملاحظة منهجية:** هذه الفجوات **لم تكن ظاهرة قبل بناء المصفوفة**. هذا بالضبط ما تُستخدم له مصفوفة التتبّع — وليس التوثيق الشكلي.
...
| F-EVL-11 | FR-U-043 | ✓ |
| F-EVL-12 | FR-P-023 | ✓ |
| **F-EVL-13** | — *(HR-001..006 سياسات لا قدرة)* | **GAP-03** |
| F-EVL-14 | FR-G-007 | ✓ |
| F-EVL-15 | FR-U-044 | ✓ |
| F-EVL-16 | FR-P-024 | ✓ |
...
| F-EVD-01 | FR-U-050 | ✓ |
| **F-EVD-02 · 05** | SR-006 · DR-012 · §8 | **GAP-04 (مقبول)** |
...
**التغطية:** 88 من 91 ميزة لها متطلب واحد على الأقل · **3 مزايا بلا متطلب** (GAP-01 · GAP-02 ×2 · GAP-03) · **2 مغطّاة بقواعد لا متطلبات** (GAP-04).
...
| CAP-10 | `DEFERRED` P2 | — |
| CAP-11 … CAP-14 | ✓ مغطّاة | FR-P-010..018 · FR-G-020 |
| CAP-15 … CAP-19 | ✓ مغطّاة | FR-U-020..034 · FR-A-002 |
| CAP-20 … CAP-26 | ✓ مغطّاة | FR-U-040..044 · FR-P-020..022 · FR-A-003..006 · FR-G-006/008/009 |
| CAP-27 · CAP-28 | 🟡 جزئية | FR-P-023/024 · FR-G-007 — **ينقص متطلب واجهة المراجع (GAP-03)** |
| **CAP-29** | 🔴 **بلا متطلب** | **GAP-02** |
...
## سجل التغييرات المطلوبة على الأساس

| Change ID | السبب | الأثر | الحالة |
|---|---|---|---|
| `CHG-001` | GAP-02 — إضافة `FR-P-036` (اشتقاق `Achievement`) | إضافة متطلب + ميزة في الجرد + تحديث المصفوفة | 🔴 **بانتظار اعتمادك** |
| `CHG-002` | GAP-03 — إضافة `FR-P-037` (واجهة المراجع وسجل القرار) | إضافة متطلب + تحديث المصفوفة | 🔴 **بانتظار اعتمادك** |
| `CHG-003` | GAP-05 — إضافة `F-GOV-12` · `F-GOV-13` · `F-AI-11` للجرد | تحديث جرد المزايا + المصفوفة | 🔴 **بانتظار اعتمادك** |
| `CHG-004` | GAP-06 — إضافة `F-CON-12` (بوابة اعتماد SME) | تحديث جرد المزايا + المصفوفة | 🔴 **بانتظار اعتمادك** |

> **لم أنفّذ هذه التغييرات.** الأساس مُجمَّد (D-042)، والتعديل عليه يحتاج قرارك — **ولا يُعدَّل بصمت**.
