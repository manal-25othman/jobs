...
> كل عقد هنا ملزم بالثوابت التسعة، وأهمها: **الوكيل يقترح ولا يكتب** (LBD-05).
...
- **Cost sensitivity:** `low` بسقف يومي صارم لكل مستخدم (الأكثر تكرارًا وأخطرها على الفاتورة) — وعند بلوغ السقف **يتدرّج ولا يتوقف** (D-014).
- **Dependencies:** A19 (المصدر الوحيد للقرار) · A18 (بوابة) · لا يعتمد على A02 مباشرة.
- **Fallback / تدرّج الميزانية (D-014):** ثلاثة أنماط متدرجة — `Full AI Mode` → `Low-cost AI Mode` (نموذج أصغر، سياق مختصر، بلا تحليل جديد) → `Deterministic Companion Mode` (بلا نموذج إطلاقًا). **لا يتوقف Companion أبدًا:** في النمط الحتمي يظل قادرًا على عرض الحالة الحالية · الخطوة التالية · التقدّم · التذكيرات · التوصيات المخزّنة · شرح القرارات القائمة. التحليل الجديد عالي التكلفة يخضع لسياسة الميزانية.
...
- **Required Context:** الثوابت التسعة · قواعد المستويات · إقرار AI · إشارات العملية.
