...
| `system_derived` **(معتمد — D-020)** | حساب حتمي من قواعد النظام على حقائق قائمة | `Gap` · `Claim.level` · أهلية `Achievement` · حسابات الحالة الحتمية | مُشتق من أدلة، وليس دليلًا بذاته |

**حقول إلزامية على كل حقيقة `system_derived` (D-020):**

| الحقل | المعنى |
|---|---|
| `derivation_rule_id` + `derivation_rule_version` | القاعدة التي أنتجت الحقيقة وإصدارها — بلا ذلك لا يمكن إعادة الحساب ولا تفسير التغيّر |
| `source_entity_refs[]` | الكيانات التي اشتُقت منها (أدلة · تقييمات · متطلبات دور) |
| `calculation_timestamp` | زمن الحساب — منفصل عن `asserted_at` |

**قيد مطلق:** `system_derived` **لا يُستخدم لأي مخرَج نموذج لغوي** مهما كان بسيطًا. مخرَج النموذج `ai_generated` دائمًا، حتى لو مرّ بقاعدة بعده.
