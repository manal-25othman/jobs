...
**pack_id:** `trk_frontend_junior` · **track_id:** `TRK-01` · **pack_version:** `0.1.0`
...
**5 مهارات أساسية فقط.** المعيار: *المهارة التي بدونها لا يُنجَز عمل الدور، ويمكن إثباتها في نشاط 60–120 دقيقة، وتُنقل معه إلى أي فريق ومكدّس.*
...
| **React** | ➖ **supporting** | **قرار مقصود:** الأساسيات الخمس تُنقل إلى Vue/Angular/Svelte؛ جعل React أساسية يربط المسار بمكدّس واحد ويخالف `ARCH-REQ-TRACK-01`. **ملاحظة سوق تحتاج تحقّقًا:** إعلانات الوظائف تطلب React كثيرًا — يُسجَّل كفرضية لا كحقيقة (§11) |
...
| `skl_git_basics` | Git في سياق فريق | `fam_engineering_practice` | supporting | familiar | medium | `rec_tools` | فروع ورسائل التزام مفهومة · تغييرات مجمّعة منطقيًا | التزام واحد ضخم · رسائل بلا معنى |
...
| `skl_git_basics` | ➖ | medium | familiar | شرط العمل في فريق | process_trace | 1 | **نعم** — بنية الالتزامات | لا (إلا عند الخلل) | **لا يُقاس في P1** ⚠️ |
...
| **C-2** | `skl_git_basics` **لا يقيسه أي نشاط** | فجوة معلنة بلا مخرج إثبات → تظهر للمستخدم ولا يستطيع سدّها | إمّا **إخراجه من الخريطة في P1** (أرجّحه) أو إضافة متطلب تسليم عبر مستودع في `act_fe_003` |
| **C-3** | `act_fe_001` يحمل 4 مهارات أساسية | خطر القياس السطحي | الرُبريك يوزن `api_integration` و`js_fundamentals` بثقل، و`semantic_html` و`css` بوزن أخف — **ولا ينتج دليلًا للأخيرتين إلا ببلوغ عتبتهما** |
...
## 5.3 `act_fe_003` — طلب تغيير على مشروع قائم + عطل جديد
...
estimated_duration_min: 110   (المدى 90–130)
ai_usage_mode: ai_assisted
evidence_strength_class: platform_controlled
high_strength: false
can_yield_verified: false
...
**business_context**
> ميزة «تصفية الطلبات» تعمل في الإنتاج. بعد اجتماع مع صاحب المصلحة، وصل **طلب تغيير**: التصفية يجب أن تدعم **اختيارًا متعددًا** بدل واحد، وأن **تبقى محفوظة عند تحديث الصفحة**. وأثناء عملك، أرسل المدير رسالة: **عميل داخلي يشتكي أن عدّاد النتائج يظهر رقمًا خاطئًا** — وهو عطل قائم قبل تغييرك.
...
```json
[
 {"file_ref":"existing_filter_project/","description":"مشروع قائم يعمل، بافتراض مضمَر بأن التصفية قيمة واحدة","is_platform_private":true,"contains_planted_issue":true},
 {"file_ref":"change_request.md","description":"طلب التغيير — **لا يحدد الأولوية عند التعارض ولا سلوك الحفظ بالتفصيل**","is_platform_private":true,"contains_planted_issue":true},
 {"file_ref":"manager_message.md","description":"رسالة المدير عن عدّاد النتائج (عطل قائم)","is_platform_private":true,"contains_planted_issue":true}
]
```

**task_brief**
1. **اقرأ الكود القائم** وحدد مكان التغيير الصحيح.
2. نفّذ الاختيار المتعدد وحفظ الحالة.
3. تعامل مع عطل العدّاد.
4. **تحقّق أن السلوك الأصلي ما زال يعمل.**
5. اكتب: ما تأثر · ما لم تنفّذه ولماذا · **أسئلتك للمدير إن وُجدت**.

**expected_deliverables**
`modified_code` · `impact_notes` (ما تأثر) · `regression_evidence` (السلوك الأصلي يعمل) · `counter_fix_or_rationale` · `questions_or_assumptions` · `ai_disclosure` — **كلها إلزامية**.
...
| كيف تُحفظ الحالة؟ | آلية معقولة مع بيان حدودها |

**related_skills**
| المهارة | الثقل |
|---|---|
| `skl_js_fundamentals` | **أساسي ثقيل** |
| `skl_api_integration_states` | **أساسي متوسط** *(التصفية تؤثر على الاستدعاء والحالات)* |
| `skl_frontend_debugging` | **أساسي متوسط** *(العدّاد)* |
| `skl_code_reading` · `skl_requirement_clarification` · `skl_form_validation` · `skl_technical_communication` | مساند |

**deterministic_checks**
```json
[
 {"check_id":"dc_003_1","check_type":"presence","rule_description":"وجود المخرجات الإلزامية الستة","is_mandatory":true},
 {"check_id":"dc_003_2","check_type":"value_match","rule_description":"الاختيار المتعدد يعمل: نتائج تطابق مجموع الفلاتر المختارة","is_mandatory":true},
 ...
 {"check_id":"dc_003_4","check_type":"constraint","rule_description":"الحالة تُحفظ وتُستعاد بعد التحديث","is_mandatory":true},
 {"check_id":"dc_003_5","check_type":"constraint","rule_description":"حالة «لا نتائج» مُعالَجة بعد التصفية","is_mandatory":true},
 {"check_id":"dc_003_6","check_type":"presence","rule_description":"وجود بيان صريح: أُصلح العدّاد أم أُبلغ عنه","is_mandatory":true}
]
```
...
```json
[
 {"check_id":"ic_003_1","type":"planted_contradiction",
  "location":"الكود القائم يفترض قيمة واحدة في ثلاثة مواضع — أحدها بعيد عن مكان التغيير الواضح",
  "expected_user_behavior":"يجد المواضع الثلاثة أو يذكر احتمال وجود مواضع أخرى ويفحص",
 ...
 {"check_id":"ic_003_2","type":"missing_information",
  "location":"change_request.md لا يحدد ما يحدث لخيار محفوظ لم يبقَ متاحًا",
  "expected_user_behavior":"يسأل أو يوثّق افتراضًا صريحًا",
  "raw_ai_output_behavior":"ينفّذ افتراضًا صامتًا",
  "weight":"high","is_blocking":true,"linked_criterion_id":"core.judgment.assumption_check"},

 {"check_id":"ic_003_3","type":"follow_up_modification",
  "location":"رسالة المدير عن العدّاد تصل بعد ~40 دقيقة من البدء",
  "expected_user_behavior":"يتعامل معها بقرار معلَّن (يصلح الآن أو يؤجل ويبلّغ) ولا يتجاهلها",
  "raw_ai_output_behavior":"يتجاهل التدخّل أو يدمجه بلا قرار معلَن",
  "weight":"high","is_blocking":false,"linked_criterion_id":"core.ambiguity.handling"},

 {"check_id":"ic_003_4","type":"edge_case",
  "location":"عطل العدّاد سببه احتساب مكرر يظهر فقط عند اختيار فلترين",
  "expected_user_behavior":"إن أصلحه، يشرح الشرط الذي يُظهره؛ وإن أجّله، يوثّق خطوات إنتاجه",
  "raw_ai_output_behavior":"يعيد كتابة العدّاد بلا فهم شرط ظهور العطل",
  "weight":"medium","is_blocking":false,"linked_criterion_id":"track.frontend.root_cause_isolation"},

 {"check_id":"ic_003_5","type":"contextual_defense_question",
  "location":"سؤال بعد التسليم",
...
`rubric_ref: rub_fe_003@0.1.0` · مراجعة بشرية **إلزامية** · `sme_approval: null` ⚠️ · `authoring_effort_minutes: TO BE MEASURED`

## 5.4 تغطية المهارات الأساسية عبر الأنشطة *(بوابة G-4)*

| المهارة الأساسية | `act_fe_001` | `act_fe_002` | `act_fe_003` | عدد الأنشطة | يبلغ `Verified`؟ |
|---|---|---|---|---|---|
| `skl_semantic_html` | خفيف | **ثقيل** | — | **2** ✅ | نعم |
| `skl_css_layout_responsive` | خفيف | **ثقيل** | — | **2** ✅ | نعم |
| `skl_js_fundamentals` | **ثقيل** | — | **ثقيل** | **2** ✅ | نعم |
| `skl_api_integration_states` | **ثقيل** | — | متوسط | **2** ✅ | نعم |
| `skl_frontend_debugging` | — | **ثقيل** | متوسط | **2** ✅ | نعم |

> ✅ **كل مهارة أساسية لها نشاطان.** لكن **ثلاثة أزواج تعتمد على وزن «خفيف» أو «متوسط»** — و`10-evidence-lifecycle` تشترط بلوغ عتبة بند المهارة، لا مجرد وجود البند.
> **⚠️ نقطة قرار لـSME:** هل الوزن الخفيف في `act_fe_001` كافٍ لإنتاج دليل ثانٍ لـ`semantic_html` و`css`؟ **رأيي: لا.** وأوصي بأحد أمرين: **رفع وزنهما في `act_fe_001`**، أو **قبول أن هاتين المهارتين تبلغان `Demonstrated` فقط في P1** والإفصاح بذلك.
...
## 6.3 `rub_fe_003` — طلب تغيير وانحدار

```
rubric_id: rub_fe_003 · rubric_version: 0.1.0 · activity_id: act_fe_003 · status: draft · sme_approval: null
```
...
| 3 | `core.integrity.error_detection` | منع الانحدار والتحقق من السلوك الأصلي | `skl_js_fundamentals` | **15%** | core |
| 4 | `core.judgment.assumption_check` | التعامل مع التعارض والمتطلب الناقص | `skl_requirement_clarification` | **15%** | core |
| 5 | `core.ambiguity.handling` | التعامل مع التدخّل أثناء العمل | `skl_technical_communication` | **10%** | core |
| 6 | `track.frontend.state_persistence_quality` | جودة حفظ الحالة واستعادتها | `skl_api_integration_states` | **10%** | track |
| 7 | `core.judgment.decision_under_uncertainty` | قرار الإصلاح مقابل التأجيل وتعليله | `skl_frontend_debugging` | **10%** | core |
...
| **L2** | **وجد المواضع الثلاثة، أو ذكر احتمال وجود مواضع أخرى وفحص فعلًا** *(`ic_003_1`)* |
| L3 | يشرح كيف بحث (تتبّع مسار البيانات لا بحث نصي أعمى) |

**3 · `core.integrity.error_detection`** — 15% · **`is_blocking: true`**
| L0 | لا دليل على فحص السلوك الأصلي |
| L1 | ذكر أنه فحص بلا دليل |
| **L2** | **دليل على أن السلوك الأصلي يعمل بعد التغيير** *(`dc_003_3`)* |
...
**5 · `core.ambiguity.handling`** — 10%
| L0 | تجاهل رسالة المدير |
| L1 | استجاب بلا قرار معلَن عن الأولوية |
| **L2** | **قرار معلَن: أصلح الآن أو أجّل وأبلغ** *(`ic_003_3`)* |
| L3 | أعاد ترتيب أولوياته وأبلغ بأثر ذلك على التسليم |

**1 · `code_comprehension`** · **6 · `state_persistence_quality`** · **7 · `decision_under_uncertainty`**
> ⚠️ **تحتاج صياغة واصفات من SME** — الإطار موجود والنص النهائي لا.
...
# 7. INTEGRITY CHECKS — ملخص مجمّع

> **الغاية: قياس الفهم والاستقلالية — لا اصطياد المستخدم.**
> يُعلَن للمستخدم مسبقًا أن الأنشطة تحوي عناصر تحقق مُصمَّمة، **بلا كشف أيٍّ منها تحديدًا** (سياسة AI §5).

| النشاط | العدد | الأنواع المستخدمة | الحاسمة |
|---|---|---|---|
| `act_fe_001` | **4** | `missing_information` · `data_inconsistency` · `outlier` · `contextual_defense_question` | 3 |
| `act_fe_002` | **4** | `planted_contradiction` · `hidden_assumption` · `edge_case` · `contextual_defense_question` | 2 |
| `act_fe_003` | **5** | `planted_contradiction` · `missing_information` · `follow_up_modification` · `edge_case` · `contextual_defense_question` | 3 |
| **المجموع** | **13** | — | 8 |

### توزيع الأنواع — وسبب التوزيع
| النوع | العدد | لماذا في هذا المسار |
|---|---|---|
| `missing_information` | 2 | **الافتراض الصامت أخطر سلوك مبتدئ** — والمتطلب الناقص واقع يومي في عمل الواجهة |
| `contextual_defense_question` | **3 (واحد في كل نشاط)** | **الأداة الأقوى ضد التسليم غير المفهوم** — ولا تُقاس بغيرها |
| `planted_contradiction` | 2 | يقيس تمييز العَرَض عن السبب وتحليل أثر التغيير |
| `edge_case` | 2 | يقيس فحص الأثر الجانبي |
| `data_inconsistency` | 1 | واقع البيانات الحقيقية — ونادرًا ما يتوقعه المبتدئ |
| `hidden_assumption` | 1 | يقيس رفض الحل السريع الضار |
| `follow_up_modification` | 1 | يقيس التعامل مع تغيّر الأولويات أثناء العمل |

### ✅ التزام بحدود السياسة
- **لا فحص يعتمد على إخفاء معلومة يحتاجها المستخدم للنجاح** — كل نقص إما يُسأل عنه أو يُوثَّق افتراضه، **والمسارَان مقبولان**.
- **لا فحص يعاقب على استخدام AI** — الفحوص تقيس التحقق والحكم، والإفصاح لا يخفض الدرجة.
- **لا فحص واحد يقرر نتيجة** — الحد الأدنى إشارتان مستقلتان قبل أي أثر على مستوى الدليل (سياسة AI §4).
- **كل فحص حاسم يقابله بند رُبريك ظاهر للمستخدم بعد التقييم** — فيعرف على أي أساس قِيس.
...
| 3 | تمارين تحويل بيانات (مصفوفات وكائنات) | **SOURCE TO VERIFY** | `exercise` | en | free | 2–4 س | **الفجوة الأكثر تكرارًا:** تحويل استجابة API إلى ما تحتاجه الواجهة | `act_fe_001` |
```
url: SOURCE TO VERIFY · quality_status: unverified
...
| 2 | أنماط حالات التحميل والخطأ والفراغ | **SOURCE TO VERIFY** | `article` | en | free | 30–45 د | **أهم فجوة في هذا المسار** — ولم أحدد مصدرًا موثوقًا بعد | `act_fe_001` |
| 3 | مشروع تطبيقي يستهلك API عامًا | **SOURCE TO VERIFY** | `project` | en | free | 4–6 س | تطبيق كامل الحلقة | `act_fe_001` |
```
...
| 3 | منهج عزل المشكلة | **SOURCE TO VERIFY** | `article` | en | free | 30 د | **المنهج أهم من الأداة** — ولا مصدر محدد بعد | `act_fe_002` |
...
| `skl_git_basics` | **SOURCE TO VERIFY** | `tutorial` | ⚠️ مرتبط بملاحظة التغطية `C-2` |
| `skl_code_reading` | **SOURCE TO VERIFY** | — | ⚠️ **لا أعرف موردًا جيدًا لهذه المهارة** — فجوة حقيقية · التعلّم الأفضل بالممارسة عبر `act_fe_003` |

**إجمالي الموارد:** 18 مدخلًا · **`unverified` بالكامل** · **6 منها بلا مزوّد محدد**.
...
## 14. حالة بوابات الجودة الآن

| البوابة | الحالة | الملاحظة |
|---|---|---|
| G-1 مصدر ومراجعة لكل سجل | 🟡 جزئي | المصادر موجودة · **لا مراجعة واحدة** |
| G-2 المهارات موجودة وفعّالة | ✅ | 14 مهارة مُعرَّفة |
| G-3 كل مهارة أساسية لها نشاط | ✅ | — |
| **G-4 كل مهارة أساسية لها نشاطان** | 🟡 | **مستوفاة شكلًا · مشكوك فيها فعلًا** (ملاحظة `C-3`) |
| G-5 النشاط المُنتج لـ`Verified` فيه I1+I2+I3 + اعتماد SME | 🔴 | **الفحوص موجودة · الاعتماد غائب** → `can_yield_verified = false` |
| G-6 رُبريك و`skill_criteria_map` لكل نشاط | ✅ | — |
| G-7 كل بند له مهارة أو مُعلَّم | ✅ | كل البنود لها `linked_skill` |
| **G-8 واصفات لكل مستوى** | 🔴 | **7 بنود بإطار بلا واصفات نهائية** |
| **G-9 معايرة ≥80% ومجموعة ذهبية** | 🔴 | **لم تُبنَ** |
| G-10 مورد لكل فجوة ذات أولوية | 🟡 | موجود · **كله `unverified`** · 6 بلا مزوّد |
| G-11 كل مورد له تطبيق | ✅ | — |
| G-12 سياسة حداثة ومقياس لكل مهارة | ✅ | فرضيات لكنها موجودة |
| G-13 إشارات السوق بحجم عيّنة | ✅ | **فارغة عمدًا** |
| G-14 قاعدة عرض لكل أصل × مستوى | ✅ | §12 |
| G-15 manifest بإصدارات السجلات | 🔴 | **السجلات العالمية لم تُبذر بعد** |
| G-16 لا محتوى AI بلا مراجع | 🔴 | **كل الحزمة `ai_assisted` بلا مراجع** |
| G-17 لا معرّف مكرر ولا مرجع معطوب | 🟡 | يحتاج فحصًا آليًا |
| G-18 اكتمال ثنائي اللغة | 🔴 | **نصوص كثيرة بلغة واحدة** |

**النتيجة: 6 بوابات حمراء · 4 جزئية · 8 مستوفاة.** → **الحزمة `draft` صالحة للمراجعة، غير صالحة للنشر.**
